...
export default function WindowInfoPlusMobile({ windowInfo, setWindowInfo }) {
  const [hov, setHov] = useState('#337202');

  return (
    <div className=" bg-white flex flex-col justify-between p-6 fixed bottom-24 z-30 w-full h-178px ..6x6:bottom-16">
      <svg
        // onTouchStart={() => setWindowInfo(false)}
        onClick={() => setWindowInfo(false)}
      ...
      <div className="flex flex-row justify-between mt-2">
        <div className=" flex flex-col gap-1">
          <p className=" font-montserrat font-medium text-13px leading-140% pr-8">
            {windowInfo.title}
          </p>
          <p className=" font-montserrat font-normal text-11px leading-140% text-gray-quick-silver">
            {windowInfo.text}
          </p>
        </div>

        <div className="flex flex-col gap-2.5">
          <p className=" whitespace-nowrap font-montserrat font-medium text-11px text-label-pink-2 leading-120%">
            {windowInfo?.price?.new}
          </p>
          <p className=" whitespace-nowrap font-montserrat font-medium text-11px leading-120% line-through">
            {windowInfo?.price?.old}
          </p>
          <p className=" text-end whitespace-nowrap font-normal text-11px leading-140% text-label-orange font-montserrat">
      ...
      <div className="flex flex-row justify-between mt-2">
        <div className="flex items-center">
          <div className="w-100px flex justify-center border-b-9px border-primary border-opacity-30 ">
            <p className="-mb-1.5 font-montserrat font-medium leading-100% text-xs">
              Подробнее
            </p>
          </div>
        </div>
        <button
          ...
          className="flex flex-row items-center font-montserrat font-medium leading-100% text-12px text-primary px-5 py-2.5 bg-gray-light2 hover:bg-primary hover:text-white"
        >
          <svg
            className=" mr-1.5"
            width="20"
